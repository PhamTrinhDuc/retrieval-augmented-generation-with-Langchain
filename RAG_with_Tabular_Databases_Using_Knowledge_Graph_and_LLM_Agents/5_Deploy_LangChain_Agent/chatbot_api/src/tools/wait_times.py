...
def _get_current_wait_time_minutes(hospital: str) -> int:
    ...
    current_hospital = _get_current_hospitals()
    if hospital.lower() not  in current_hospital:
        return -1
    return np.random.randint(0, 120)
...
def  get_current_wait_times(hospital: str) -> str:
    ...
    if wait_time_in_minutes == -1:
        return f"Hospital {hospital} does not exits"
